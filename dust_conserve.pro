...
for sim=0,n_elements(sims)-1 do begin
  ...
  dst_fre_atm=fltarr(1) & dst_cor_atm=fltarr(1) & dst_fre_gnd=fltarr(1) & dst_cor_gnd=fltarr(1)	
  ...
  dst_fre_atm_globally_integrated=fltarr(n_records) & dst_cor_atm_globally_integrated=fltarr(n_records)
  dst_fre_gnd_globally_integrated=fltarr(n_records) & dst_cor_gnd_globally_integrated=fltarr(n_records)
  ...
  for fnum=fort11_1,fort11_2 do begin
	...
	;Calculate total water in the atmosphere and on the ground
        dst_fre_atm=0.0*dst_fre_atm
        dst_cor_atm=0.0*dst_cor_atm
        dst_fre_gnd=0.0*dst_fre_gnd
        dst_cor_gnd=0.0*dst_cor_gnd
       
        for k=0,59 do begin
          for j=lat1,lat2 do begin
            for l=0,23 do begin
              dst_fre_atm=dst_fre_atm+(dxyp[j])*((dsig4(l))*p4[j,k]*100.0/grav[0])*qtrace4(j,k,l,0)
              dst_cor_atm=dst_cor_atm+(dxyp[j])*((dsig4(l))*p4[j,k]*100.0/grav[0])*qtrace4(j,k,l,4)
            endfor
            dst_fre_gnd=dst_fre_gnd+(dxyp[j])*qcond4[j,k,0]
	    dst_cor_gnd=dst_cor_gnd+(dxyp[j])*qcond4[j,k,4]      
          end
        endfor

        dst_fre_atm_globally_integrated[count11]=dst_fre_atm
        dst_cor_atm_globally_integrated[count11]=dst_cor_atm
        dst_fre_gnd_globally_integrated[count11]=dst_fre_gnd
        dst_cor_gnd_globally_integrated[count11]=dst_cor_gnd
  ...
  dst_fre_atm_globally_integrated=dst_fre_atm_globally_integrated[where(year_array eq 19)]
  dst_cor_atm_globally_integrated=dst_cor_atm_globally_integrated[where(year_array eq 19)]
  dst_fre_gnd_globally_integrated=dst_fre_gnd_globally_integrated[where(year_array eq 19)]
  dst_cor_gnd_globally_integrated=dst_cor_gnd_globally_integrated[where(year_array eq 19)]
  lsa=lsa[where(year_array eq 19)]
  dst_tot_atm_globally_integrated=dst_fre_atm_globally_integrated+dst_cor_atm_globally_integrated
  dst_tot_gnd_globally_integrated=dst_fre_gnd_globally_integrated+dst_cor_gnd_globally_integrated
  dst_tot_all_globally_integrated=dst_tot_atm_globally_integrated+dst_tot_gnd_globally_integrated

  ymin=min([dst_tot_atm_globally_integrated,dst_tot_gnd_globally_integrated,dst_tot_all_globally_integrated])
  ymax=max([dst_tot_atm_globally_integrated,dst_tot_gnd_globally_integrated,dst_tot_all_globally_integrated])
...
percent_change=100.0*(dst_tot_all_globally_integrated[-1]-dst_tot_all_globally_integrated[0])/dst_tot_all_globally_integrated[0]

cap_equiv=100.0*( (dst_tot_all_globally_integrated[-1]-dst_tot_all_globally_integrated[0])/(dxyp[0]*60.0+dxyp[1]*60.0) )/900.0

;xyouts,925,160,string(percent_change,' %'),charsize=4,font=1,/device

print,sims[sim],types[type],dst_tot_gnd_globally_integrated[-1]-dst_tot_gnd_globally_integrated[0],dst_tot_atm_globally_integrated[-1]-dst_tot_atm_globally_integrated[0],percent_change,dst_tot_all_globally_integrated[-1]-dst_tot_all_globally_integrated[0],cap_equiv
