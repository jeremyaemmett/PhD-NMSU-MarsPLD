...
years=[19]
sims=['O15_RIC_CAP','O15_RIC_MID_40','O20_RIC_CAP','O20_RIC_MID_40','O25_RIC_CAP','O25_RIC_MID_40','O30_RIC_CAP','O30_RIC_MID_40','O35_RIC_CAP','O35_RIC_MID_40','O40_RIC_CAP','O40_RIC_MID_40','O45_RIC_CAP','O45_RIC_MID_40','PRES_RIC_047']
...
;   SPAWN,'./GCMextract_FB'
;
;   SPAWN,'mv fort.40 fort.40_'+sims[sim]
;   SPAWN,'mv fort.41 fort.41_'+sims[sim]
;   SPAWN,'mv fort.42 fort.42_'+sims[sim]
;   SPAWN,'mv fort.43 fort.43_'+sims[sim]
;
...
season_counter=f8a
   ...
   tinterpa=fltarr(36,60,n_elements(pinterp),12)
   uinterpa=fltarr(36,60,n_elements(pinterp),12)
   minterpa=fltarr(36,60,n_elements(pinterp),12)
   finterpa=fltarr(36,60,n_elements(pinterp),12)
   cinterpa=fltarr(36,60,n_elements(pinterp),12)
   vinterpa=fltarr(36,60,n_elements(pinterp),12)
   iinterpa=fltarr(36,60,n_elements(pinterp),12)
   season_id=fltarr(1)
   ...
   for fnum=fort11_1,fort11_2 do begin
	 ...
	 if (lsa[count11] mod 30) lt (lsa[count11-2] mod 30) and fix((lsa[count11] mod 30)) eq 0 and fix(lsa[count11]/360.0) eq years[0] then begin
           if fix((lsa[count11] mod 360)/10.) eq 0 then season_id=0
           if fix((lsa[count11] mod 360)/10.) eq 3 then season_id=1
           if fix((lsa[count11] mod 360)/10.) eq 6 then season_id=2
           if fix((lsa[count11] mod 360)/10.) eq 9 then season_id=3
           if fix((lsa[count11] mod 360)/10.) eq 12 then season_id=4
           if fix((lsa[count11] mod 360)/10.) eq 15 then season_id=5
           if fix((lsa[count11] mod 360)/10.) eq 18 then season_id=6
           if fix((lsa[count11] mod 360)/10.) eq 21 then season_id=7
           if fix((lsa[count11] mod 360)/10.) eq 24 then season_id=8
           if fix((lsa[count11] mod 360)/10.) eq 27 then season_id=9
           if fix((lsa[count11] mod 360)/10.) eq 30 then season_id=10
           if fix((lsa[count11] mod 360)/10.) eq 33 then season_id=11
	    for k=4,nlevsm1,2 do begin
	       l=(k-1-3)/2
	    ...
	    ;the atmospheric temperature at the 0.5 mbar pressure level above that gridpoint
         ...
         SAVE,tinterpa,filename=string('xsections/',sims[sim],'_','Temp','_xsections.sav')
	    ...
	    xinterpa=iinterpa
         ...
         if x ge 0 and x le 3 then SAVE,xinterpa,filename=string('xsections/',sims[sim],'_',xname,'_xsections.sav')
         if x eq 4 or x eq 5 then SAVE,xinterpa,filename=string('xsections/',sims[sim],'_',xname,'_xsections.sav')
